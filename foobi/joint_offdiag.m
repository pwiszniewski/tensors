function [ V , D ] =  joint_offdiag(A,jthresh)
...
% * threshold is an optional small number (typically = 1.0e-8 see the
% M-file).
%
% Output :
...
encore  = 1; 
cnt = 0;
fprintf(stdout,'Joint off-diag reached. Matrix size %d, nmat %d\n',m,nm/m);
fflush(stdout);
...
fprintf(stdout, 'sweeps: %d, diagonal norm: %4.4e, s %4.4e\n',cnt,dnorm,s);
D = A ;
...
%  HTML        =
%  "ftp://sig.enst.fr/pub/jfc/Papers/joint_diag_pert_an.ps",
